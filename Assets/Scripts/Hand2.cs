...
public class Hand2 : MonoBehaviour {
	...
	private Atom2 FindClosestAtom() {
		float smallestDistance = float.PositiveInfinity;
		Atom2 closestAtom = null;
		foreach (Atom2 anAtom in Atom2.allAtoms) {
	...
	private void ReleaseAtom () {
		this.atom.GetComponent<Atom2> ().SetIsBeingHeld(false);
		this.atom = null;
	}
	...
	private void HandleAtoms () {
		if (this.atom != null) {
			this.atom.GetComponent<Atom2> ().makeRoot ();
